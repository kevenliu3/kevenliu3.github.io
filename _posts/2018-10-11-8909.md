...
public static String updateApk8909(String commond1, String commond2, String commond3) {
    ...
    HiBoxDoor.DownloadService.onStartCommand()
    else {
                        // TEST FOR MSM8909
        ...
        HiBoxMsg.DownloadService.onStartCommand()
        else {
                        // TEST FOR MSM8909
                        BaseConfigs.Logger.info(TAG, "下载完成，PM将升级:" + apkFilePath + apkName);
                        ...
                        // ApkController.restartApp(getApplicationContext(), "shop.hibox.vip.door");
                    }
```

7. 不支持`SmdtManager`相关API
...
if (!BaseConfigs.IS_MSM8909) {
            ...
            MqttService.getLightIsBroken()
             if (BaseConfigs.IS_MSM8909) {
            // TODO: 2018/8/29 待确认8909是否需要
            return GpioUtil8909.readIO(10) == 1;
        }

 MqttService.sendHeartbeat()
 if (!BaseConfigs.IS_MSM8909)
                            SmdtManager.setBackLight(hasPower ? 1 : 0);       
```

8. 如果需要和rk3288在同一套代码，可考虑使用全局变量控制`BaseConfigs.IS_MSM8909`逻辑的走向
